...
export default function UsernameOTPChoice(props: LoginComponentProps) {
  ...
  const handleSendOtpWithDeliveryMethod = async (deliveryMethod: DeliveryMethodType) => {
    setIsLoading(true);
    ...
    try {
      ...
      setCurrentLoginField(currentUsername.trim());
      ...
      {isLoadingOTPOptions ? (
        ...
        <>
          ...
          {selectedDeliveryMethod && (
            colorScheme === "light" ? (
              <Pressable
                onPress={() => handleSendOtpWithDeliveryMethod(selectedDeliveryMethod)}
